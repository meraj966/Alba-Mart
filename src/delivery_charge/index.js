...
function DeliveryCharges() {
  ...
  const handleDelete = async (id) => {
    ...
    if (result.isConfirmed) {
      ...
      Swal.fire(
        "Deleted!",
        "Your delivery charge has been deleted.",
        "success"
      );
    }
  };
  ...
  const modal = () => (
    <Modal
      onClose={() => setAddNewDeliveryCharge(false)}
      open={addNewDeliveryCharge}
    >
    ...
    <>
      <Stack direction="row" spacing={2} className="my-2 mb-2">
        <Typography
          ...
          sx={{ flexGrow: 1 }}
        ></Typography>
        {userHasAccessToKey(userInfo, CONTROL_ADD_DELIVERY_CHARGES) ? (
          <Button
            variant="contained"
