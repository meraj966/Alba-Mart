    ...
    const fetchData = async () => {
      try {
        ...
        if (categoryDoc.exists()) {
          ...
          const subcategories = Object.keys(subCategoryMap).map(
            (subcategoryName) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        if (categoryDoc.exists()) {
          ...
          Swal.fire(
            "Deleted!",
            `${row.subcategoryName} has been deleted.`,
            "success"
          );
        }
      } catch (error) {
          ...
          <TableBody>
            {subcategoryData.map((row, index) => (
              <TableRow hover role="checkbox" tabIndex={-1} key={index}>
                ...
                <TableCell align="left">
                  {userHasAccessToKey(userInfo, CONTROL_MODIFY_SUBCATEGORY) ? (
                    <>
                      <EditIcon
